...
function ts_yte_generate_file_tickets( $ts_response, $order_id ) {
	$is_dir_created = wp_mkdir_p( TS_YTE_UPLOADPATH . '/' . $order_id . '/' );
	if ( ! $is_dir_created ) {
		return array(
			'status'  => 'failure',
	...
	foreach ( $ts_response as $index => $ticket ) {
		$decoded_barcode = ts_yte_decode_barcode( $ticket['encrypted_data'] );
		...
		$generated_file = ts_yte_generate_file_ticket(
...
function ts_yte_generate_file_ticket( $name, $description, $price, $sensitive_decoded, $order_id, $index ) {
	$file_generator = new TS_YTE_HTML_Generator();

	$ticket_file_abs_path = TS_YTE_UPLOADPATH . '/' . $order_id . '/' . $index . '.' . $file_generator->extension;
	$ticket_file_url_path = TS_YTE_UPLOADURLPATH . '/' . $order_id . '/' . $index . '.' . $file_generator->extension;
...
function ts_yte_send_file_tickets_by_mail( $target_user_mail, $ticket_file_abs_paths ) {
	if ( ! empty( $ticket_file_abs_paths ) ) {
		$headers   = array( 'Content-Type: text/html; charset=UTF-8' );
		$mail_sent = wp_mail( $target_user_mail, ts_yte_get_option( 'email_subject', '' ), ts_yte_get_option( 'email_body', '' ), $headers, $ticket_file_abs_paths );
