...
function generate_file_tickets( $ts_response, $order_id ) {
	...
	foreach ( $ts_response as $index => $ticket ) {
		...
		$generated_file = generate_file_ticket(
...
function generate_file_ticket( $name, $description, $price, $sensitive_decoded, $order_id, $index ) {
	...
	$ticket_file_abs_path = WOO_TS_UPLOADPATH . '/' . $order_id . '/' . $index . '.' . $file_generator->extension;
	$ticket_file_url_path = WOO_TS_UPLOADURLPATH . '/' . $order_id . '/' . $index . '.' . $file_generator->extension;
